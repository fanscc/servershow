<template>
  <div @click='searchBlur'>
        ...
        <div class="searchCont">
          ...
          <div class="search_form" style="position: relative;">
            ...
            <div class="searchTerm">
              <input class="search_text" type="text" v-model="searchName" placeholder="请输入楼盘名称/区域" name="text" @focus='historyCotEvent' @keyup='autocomplete' />
            ...
            <div class="history_cot" style="position: absolute;width: 96%;" v-show="historyShowCondition" >
              ...
              <ul v-if="showHistory.length">
                  <a target="_blank" v-for="(item,index) in showHistory" :href="`/sh-xf/?wd=${item}`" ><li class="pg_href">{{item}}</li></a>
              </ul>
            </div>
          </div>
            <!-- 热门搜索 -->
    ...
    <div class="server wrapper clearfix pg_server1" v-show="serverCondition">
      <dl>
          <dt class="overAll"><img src="~assets/img/indexIcon1.png" /></dt>
          <dd><span>房源全面</span>别墅/公寓 覆盖全浦东豪宅</dd>
      </dl>
      <dl>
          <dt class="real"><img src="~assets/img/indexIcon2.png" /></dt>
          <dd><span>真实可信</span>专人审核，真房源100%</dd>
      </dl>
      <dl>
          <dt class="serve"><img src="~assets/img/indexIcon3.png" /></dt>
          <dd><span>专业服务</span>顾问平均掌握16个楼盘</dd>
      </dl>
      <dl>
          <dt class="highEnd"><img src="~assets/img/indexIcon4.png" /></dt>
          <dd><span>专注豪宅</span>我们始终坚持 只做豪宅</dd>
      </dl>
    </div>
     <!-- 二手豪宅 -->
    <div class="mansion">
        ...
        <div class="imgShow clearfix">
            <dl v-for="(item,index) in secondHandRecommend" :key="index">
              <a :href="`/sh-esf/${item.housedelId}.html`" target="_blank" >
                <dt>
                  <img v-lazy="item.titleImgUrl"  :alt="`${item.resblockName}二手豪宅`" :title="`${item.resblockName}二手豪宅`"
          ...
          <div class="left">
            <dl>
              <a :href="`/sh-xf/lp${projectRecommend[0].resblockId}-${projectRecommend[0].pkid}.html`" target="_blank">
                <dt>
                  <img src="~assets/img/default2.png"  v-lazy="projectRecommend[0].titleImg"
                    :alt="`${projectRecommend[0].resblockName}${projectRecommend[0].houseType}`" :title="`${projectRecommend[0].resblockName}${projectRecommend[0].houseType}`" >
                </dt>
                <dd>
                  <p class="clearfix">
          ...
          <div class="right">
              <dl>
                <a :href="`/sh-xf/lp${projectRecommend[1].resblockId}-${projectRecommend[1].pkid}.html`" target="_blank">
                  <dt>
                    <img src="~assets/img/default2.png"  v-lazy="projectRecommend[1].titleImg"
                    :alt="`${projectRecommend[1].resblockName}${projectRecommend[1].houseType}`" :title="`${projectRecommend[1].resblockName}${projectRecommend[1].houseType}`" />
                  </dt>
                  <dd>
                    <p class="clearfix">
              ...
              <dl>
                <a :href="`/sh-xf/lp${projectRecommend[2].resblockId}-${projectRecommend[2].pkid}.html`" target="_blank">
                  <dt>
                    <img src="~assets/img/default2.png" v-lazy="projectRecommend[2].titleImg"
                    :alt="`${projectRecommend[2].resblockName}${projectRecommend[2].houseType}`" :title="`${projectRecommend[2].resblockName}${projectRecommend[2].houseType}`" />
                  </dt>
                  <dd>
                    <p class="clearfix">
          ...
          <dl v-for="(item,index) in consultantInfo" :key="index">
            <dt>
              <a href="javascript:void(0);" style="cursor: default;">
                <img class="lazy_img" src="~assets/img/toux.jpg" v-lazy='item.userImgUrl'>
              </a>
            </dt>
            <dd>
              <div>
...
export default {
  ...
  methods: {
    ...
    autocomplete: function () {
      axios.get('/api/landz-web-house/secondHand/getAssociateResblock?cityCode=110000&resblockName=' + this.searchName)
        .then((res) => {
          console.log(res)
        })
    },
